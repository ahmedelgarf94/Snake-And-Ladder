...
public:
	...
	void EditableCard(Card*& pCard,CellPosition& cardPosition);		//Garf
	Card* GetCardPointer(CellPosition,int);		//Garf
	bool IsThereACard(const CellPosition position, int &Clicked_Card_Number);		//Garf

	Card* hasacard(CellPosition);		//DINA
	void notadvancecurrentplayer();     //DINA  //ADDED   //Function used in CardSeven to make the current player still to make another roll dice     
	void RemoveAllObjects();

	void newGame();		//A function that reset the interface, the players' position, wallet, and turnCounter, and starts from player 0 (i.e.: the first player).
	Cell* gCell(CellPosition &) const;
	bool overLapping(GameObject* pNew) const;
